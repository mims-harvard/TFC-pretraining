...
- In finetuning on Epilepsy (lr= 0.0005, epoch=20, batchsize=60), the finetuning set is still 60 samples (30 positive + 30 negative). There are 20 val and 11420 test samples. But we have resplit the Epilepsy dataset (i.e., regenerated the 60 finetuning set) to test the stability of the model. The code to resplition is avialiable at `TFC-pretraining/code/TFC/Data_split.py` and the splitted dataset is uploaded to this repo `TFC-pretraining/datasets/Epilepsy/` (it will also be synchronized to Figshare). 
